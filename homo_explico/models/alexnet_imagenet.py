...
def compute_induced_filtration_parallel(x, hiddens, params, percentile=0, stride=1):
    ...
    for c in range(num_channels):
        ...
        r = pool.apply_async(first_layer, args=(x[c],p,l,c,percentiles[l],stride,nlc), callback=collect_result)
    pool.close()
    pool.join()
    ...
    for c in range(num_channels):
        p = params[l].weight.data[:,c,:,:]
        h1 = h[c,:,:]
        r = pool.apply_async(mid_conv, args=(h1,p,l,c,percentiles,stride,nlc), callback=collect_result)
    pool.close()
    pool.join()
